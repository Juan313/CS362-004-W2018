...
void generateRandomGameState(struct gameState* G, int player, bool isValidCard){
  int i = 0;
  for (; i < sizeof(struct gameState); ++i){
      ((char*)G)[i] = floor(Random()*256); 
  }

  if (MY_DEBUG == 1) {
    G->handCount[player] = 15;
    G->deckCount[player] = 0;
    G->discardCount[player] = 16;
